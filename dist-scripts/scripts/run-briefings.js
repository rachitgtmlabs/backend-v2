...
const briefing_service_1 = require("../src/briefing/briefing.service");
const organizations_service_1 = require("../src/organizations/organizations.service");
dotenv_1.default.config({ path: path_1.default.resolve(process.cwd(), '.env') });
async function main() {
    const log = new common_1.Logger('run-briefings');
    const onlyOrgId = process.argv[2]?.trim();
    const app = await core_1.NestFactory.createApplicationContext(app_module_1.AppModule, {
        logger: ['error', 'warn', 'log'],
    });
    try {
        const briefings = app.get(briefing_service_1.BriefingService);
        ...
        for (const org of orgs) {
            ...
            log.log(`${org.orgId} (${org.name}): ${briefing.stats.leasesChecked} leases, ` +
...
//# sourceMappingURL=run-briefings.js.map