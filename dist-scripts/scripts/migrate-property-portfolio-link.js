...
async function main() {
    ...
    const coll = mongoose_1.default.connection.collection('properties');
    const legacyFilter = {
    ...
    const portfolioColl = mongoose_1.default.connection.collection('portfolios');
    const portfolioIds = (await portfolioColl.find({}).project({ portfolioId: 1 }).toArray()).map((d) => String(d.portfolioId ?? ''));
    const distinctPropertyPortfolioIds = (await coll.distinct('portfolio_id', {
        portfolio_id: { $exists: true, $nin: [null, ''] },
    })).map((id) => String(id));
    const orphanPortfolioIds = distinctPropertyPortfolioIds.filter((p) => p && !portfolioIds.includes(p));
    const portfoliosWithNoPropertyRow = portfolioIds.filter((p) => p && !distinctPropertyPortfolioIds.includes(p));
    console.log(JSON.stringify({
    ...
    if (propertiesTotal === 0) {
        ...
        console.log('\nMigration skipped: portfolio_id is already set on properties. The UI shows 0 because no property row uses the same id as these portfolios:', portfoliosWithNoPropertyRow.join(', ') || '(none)');
    }
    else if (needLink === 0) {
...
//# sourceMappingURL=migrate-property-portfolio-link.js.map