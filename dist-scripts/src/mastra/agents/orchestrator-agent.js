...
const SKILLS_DIR = (0, path_1.join)(__dirname, '..', 'workspace', 'skills');
function loadSkills() {
    let dirs;
    try {
        dirs = (0, fs_1.readdirSync)(SKILLS_DIR).filter((name) => {
            try {
                return (name.startsWith('task-plan-') &&
                    (0, fs_1.statSync)((0, path_1.join)(SKILLS_DIR, name)).isDirectory());
            }
            catch {
                return false;
            }
    ...
    for (const dir of dirs.sort()) {
        const skillPath = (0, path_1.join)(SKILLS_DIR, dir, 'SKILL.md');
        try {
...
function toolNameList() {
    return registry_1.TOOL_DIRECTORY.map((t) => `- ${t.name}${t.isDynamic ? '  (DYNAMIC, leaf-only)' : ''}`).join('\n');
}
exports.orchestratorAgent = new agent_1.Agent({
    id: 'lease-orchestrator-agent',
    name: 'Lease Orchestrator',
    description: 'Decomposes a user question about leases/portfolios/properties/tasks/CAM into a parallel task graph of tool calls. Uses pre-loaded domain skill files for routing.',
