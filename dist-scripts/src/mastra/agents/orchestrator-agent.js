...
function toolCatalog() {
    return registry_1.TOOL_DIRECTORY.map((t) => `- ${t.name}${t.isDynamic ? ' (DYNAMIC, leaf-only)' : ''}\n    inputs: ${t.inputs}\n    when: ${t.when}`).join('\n');
}
exports.orchestratorAgent = new agent_1.Agent({
    id: 'lease-orchestrator-agent',
    name: 'Lease Orchestrator',
    description: 'Decomposes a user question about leases/portfolios/properties into a parallel task graph of tool calls.',
