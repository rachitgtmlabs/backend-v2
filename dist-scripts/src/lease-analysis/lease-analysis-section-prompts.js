...
exports.OPERATIONAL_GUARDRAILS_B_TAIL = exports.OPERATIONAL_GUARDRAILS_A_TAIL = exports.SECTION_USER_TAIL = exports.LEASE_ANALYSIS_SYSTEM_PROMPT = void 0;
exports.LEASE_ANALYSIS_SYSTEM_PROMPT = `You are an expert commercial real estate lease analyst. The user message starts with raw OCR from one lease PDF with page markers (e.g., [PAGE 1], [PAGE 2]), then "---", then a short task for a single abstract section.
...
exports.SECTION_USER_TAIL = {
    executiveSummary: 'Section: Executive Summary. Write a concise Markdown brief (200-350 words) summarizing this lease for a busy operator. ' +
    ...
    spaceAndPremises: 'Section: Space & Premises. Extract 13 structured fields describing the leased premises. ' +
    ...
    executiveIdentity: 'Section: Executive Identity. Extract ONLY the legal entity names for landlord/lessor (leaseFrom) and tenant/lessee (leaseTo) — return the company or person name only (e.g., "PH Office II, LLC" or "Northbrook Outfitters, Inc."), NOT descriptions, definitions, commencement date formulas, or explanatory text about them. Also extract property or suite, lease identifier, rentable area (square feet as number only), rent per square foot (rate only, e.g., "$52.00"), base rent description, security deposit (amount only in "amount" field, any conditions/notes in "conditions" field), and renewal/options language.',
    financialStack: 'Section: Financial Stack. For summaryCards: return AT MOST 4 cards representing the most important financial metrics (e.g., Lease Term, Base Rent Year 1, Security Deposit, Annual Escalation). Each card has title (short label), numericValue (raw number ONLY — no currency symbols, no "months"/"years" text, no commas), valueUnit one of months|years|usd|percent|plain, and citation (short only, e.g. "p. 4" or "Art. 1"; never long legal prose; use "" if unknown). Use usd for dollar amounts as a number (76000 not "$76,000"). Use percent for displayed percent (4.65 means 4.65%). Prefer months for primary lease term length. For rentSchedule: monthlyRent and annualRent are strings in JSON (e.g. "$76,000" and "$912,000" or "76000" / "912000"); never output them as numeric literals. In rentSchedule "notes", write "Per square foot" in full (not "/sf" or "/psf" abbreviations). Also extract additionalCharges (CAM, taxes, insurance, etc.). ' +
    ...
    legalNuances: 'Section: Legal Nuances. Produce an audit-style RISK REGISTER flagging items that require human legal review. Return riskRegister with three sub-fields: counts, overallCertainty, sections. ' +
...
const PROVISION_FIELD_INSTRUCTIONS = 'For EACH topic populate three fields exactly: ' +
    '(1) synopsis.value — one plain-English sentence (<=140 chars) capturing the rule; ' +
    '(2) keyParameters.value — a short newline-separated list of concrete, quantitative or rule-level parameters (thresholds, dollar caps, business-hour ranges, approval timelines, day counts, percentages, named systems). Format as "Label: value" pairs, one per line. Use "" if no quantitative parameters exist; ' +
    '(3) narrative.value — a 2-4 sentence operator-facing explanation: WHO must do WHAT, with WHICH consents/exceptions, and the practical operational consequence. Avoid restating the synopsis verbatim. ';
const PROVISION_CLOSING_INSTRUCTIONS = 'For every field populate citation, pageReference, and amendments (empty if none). Set topic-level certainty: "high" only when the OCR directly states the rule, "medium" when inferred from related clauses, "low" when the OCR is silent or ambiguous. ' +
...
exports.OPERATIONAL_GUARDRAILS_A_TAIL = 'Section: Operational Guardrails (Part A — 14 topics). Produce structured per-topic provisions for the 14 topics listed below. ' +
...
exports.OPERATIONAL_GUARDRAILS_B_TAIL = 'Section: Operational Guardrails (Part B — 14 topics). Produce structured per-topic provisions for the 14 topics listed below. ' +
    PROVISION_FIELD_INSTRUCTIONS +
    'Topic scope (these 14 only): ' +
    'hazardousMaterials = prohibited substances, disclosure, indemnity, pre-existing conditions; ' +
    'rulesAndRegulations = building rules, Landlord right to amend, enforcement; ' +
    ...
    'subordination = subordination to existing/future mortgages, non-disturbance, attornment. ' +
    PROVISION_CLOSING_INSTRUCTIONS;
//# sourceMappingURL=lease-analysis-section-prompts.js.map