/**
 * Per-page bill-extraction prompt + JSON schema for Groq structured output.
 *
 * The caller runs OCR on the whole PDF then invokes this once per page so the
 * model sees a single document at a time. Each page is first classified as
 * `bill`, `invoice`, or `unknown`; only `bill` rows are persisted by the
 * upload service (invoices/unknowns are counted and surfaced in the UI).
 *
 * Caller supplies the list of valid expense categories for this portfolio so
 * the model can map vendor descriptions ("Tulsa Municipal Water") → category
 * name ("Water & Sewer") without inventing values.
 */
...
export interface BillExtractionResult {
  ...
  /** Short human-readable reason for the classification — surfaced in logs. */
...
export const BILL_EXTRACTION_SYSTEM_PROMPT = `You classify a single page of a scanned document and, if it is a vendor bill, extract its structured fields.
...
export function buildBillExtractionUserMessage(
  pageText: string,
  ...
  return [
    ...
    "Page OCR text:",
    "---",
    pageText.slice(0, 12_000), // safety cap
    "---",
